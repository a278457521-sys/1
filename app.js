(function () {
  ...
  function memoryMapHtml(title, branches, total) {
    ...
    return sectionHtml("图", "记忆点思维导图", `先记三条主干，再把关键词扩成完整句；参考总分 ${total} 分`, `<div class="memory-map" role="img" aria-label="${escapeHtml(`${title}记忆导图。${accessible}`)}">
      ...
      <div class="memory-branches">${branches.map((branch) => `<article class="memory-node ${escapeHtml(branch.kind)}"><div><strong>${escapeHtml(branch.label)}</strong><span>${escapeHtml(branch.score)} 分</span></div><p>${highlight(branch.text)}</p></article>`).join("")}</div>
    </div>`);
  }
